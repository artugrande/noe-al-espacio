...
export function MuteButton() {
  ...
  return (
    <button
      onClick={toggleMute}
      className={`fixed top-2 right-2 md:top-4 md:right-4 z-50 text-white p-2 md:p-3 rounded-full transition-all duration-300 text-sm md:text-base ${
        isAudioReady ? "bg-black/50 hover:bg-black/70" : "bg-gray-600/50 hover:bg-gray-500/70"
      }`}
      title={!isAudioReady ? "Audio cargando..." : isMuted ? "Activar música" : "Silenciar música"}
    >
      {!isAudioReady ? (
